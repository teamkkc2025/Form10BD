...
def clean_uid(uid_str):
    """
    Remove hidden control characters AND the literal 'x000D' suffix that
    Excel/openpyxl injects when a cell contains an embedded carriage-return.

    The literal suffix appears because openpyxl represents CR (U+000D) as the
    escape sequence x000D inside the XML, and when the string is read the
    escape leaks out as the five printable characters x 0 0 0 D appended
    right after the last real character (e.g. 'AKBPD8658G' + 'x000D').
    Simply stripping non-alphanumeric chars does NOT help because x, 0, D are
    all alphanumeric.

    Fix strategy (applied in order):
      1. Remove all real ASCII control chars (x00-x1F, x7F).
      2. Remove every occurrence of the literal pattern x0{1,}D (case-insensitive)
         that appears anywhere in the string - this catches x0D, x00D, x000D, etc.
      3. Strip anything that is not A-Z / a-z / 0-9.
    """
    s = str(uid_str)
    # Step 1 - real control characters
    s = re.sub(r'[\x00-\x1F\x7F]', '', s)
    # Step 2 - literal escaped CR sequences (x0D / x00D / x000D ...)
    s = re.sub(r'x0+[Dd]', '', s, flags=re.IGNORECASE)
