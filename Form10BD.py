...
.info-box {
    ...
    padding: 14px 20px; font-size: 13px; color: #2D4A1E;
...
def strip_all_special(value):
    """
    Removes every character that is NOT a letter, digit, or space.
    Works on any value type — safely converts to string first.
    """
    if value is None:
        return ''
    text = str(value)
...
def clean_uid_string(value):
    """Strip everything except alphanumeric from a UID."""
    if value is None:
        return ''
...
def validate_uid_row(row):
    """Validate and correct UID + ID Code for one row."""
    uid      = row.get('Unique Identification Number', '')
    id_code  = str(row.get('ID Code', '')).strip().title()
    change   = ''

    uid_str = str(uid).strip() if uid is not None else ''

    if uid_str == '' or uid_str.lower() in ('nan', 'not available', 'na', 'n/a', 'none'):
        return pd.Series(['Permanent Account Number', 'NNNNN0000N', 'Filled default PAN for missing UID'])

    uid_clean    = clean_uid_string(uid_str)
    ...
    if uid_clean.isdigit() and len(uid_clean) == 12:
        ...
        is_valid     = True
        # Store as integer string — Excel writer will convert to number
        uid_clean    = uid_clean  # keep as string here; Excel writer handles int
    ...
    elif uid_str != uid_clean:
        change = 'Formatted UID'

    return pd.Series([correct_code, uid_clean, change])


# ============================================================
#  MAIN PROCESSOR  — called fresh every time button is clicked
# ============================================================
...
def process_dataframe(raw_df):
    ...
    df[['ID Code', 'Unique Identification Number', 'Change Note']] = \
        df.apply(validate_uid_row, axis=1)
    ...
    #    We do this LAST so nothing re-introduces special chars.
    do_not_strip = {
    ...
    for col in df.columns:
        if col in do_not_strip:
            continue
        # Apply to every cell regardless of dtype
...
def to_excel_bytes(df):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Convert 12-digit Aadhaar strings to integers
        uid_series = export['Unique Identification Number']
        for i, val in enumerate(uid_series):
            s = str(val).strip()
            if s.isdigit() and len(s) == 12:
                export.at[export.index[i], 'Unique Identification Number'] = int(s)

        export.to_excel(writer, sheet_name='CleanedData', index=False,
        ...
        uid_idx  = export.columns.get_loc('Unique Identification Number')
        num_fmt  = wb.add_format({'num_format': '0'})
        txt_fmt  = wb.add_format()

        for r, val in enumerate(export['Unique Identification Number'], start=1):
            try:
                if isinstance(val, int) or (isinstance(val, str) and val.isdigit()):
                    ws.write_number(r, uid_idx, int(str(val)), num_fmt)
                else:
                    ws.write(r, uid_idx, str(val), txt_fmt)
            except Exception:
                ws.write(r, uid_idx, str(val), txt_fmt)
...
<div class="info-box">
    ...
    Only letters (A-Z), digits (0-9) and spaces are kept — commas, periods, hyphens, slashes,
    brackets, currency symbols and all other special characters are removed &nbsp;·&nbsp;
...
if uploaded_file is not None:
    try:
        # Read fresh every render — no session state caching that could serve stale data
        ...
        if "processed_df" in st.session_state and \
           st.session_state.get("file_name") == uploaded_file.name:
            ...
            st.success(
                ...
                f"all special characters removed from every column."
            )
        ...
        st.exception(e)          # shows full traceback on cloud for debugging

else:
    # Clear stale session state when a new file is not yet uploaded
